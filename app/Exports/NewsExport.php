...
class NewsExport implements FromCollection, WithHeadings, ShouldAutoSize, WithEvents
{
    ...
    {
        return [
            "TEXT", "TITLE", "IMPORTANT","EFFECT","COMMENT","INSTRUMENT"
        ];
    }
        ...
        foreach ($news as $new)
        {
            foreach($details as $detail)
            {
                ...
                if($detail->news_id==$new->id){
                    ...
                    if($detail->effect==0)
                    $col1=[
                        [$new->text, $new->title,$detail->important,"0",$detail->comment,$tags[$detail->id]]
                    ];
                    else
                        $col1=[
                            [$new->text, $new->title,$detail->important,$detail->effect,$detail->comment,$tags[$detail->id]]
                        ];
