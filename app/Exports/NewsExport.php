...
class NewsExport implements FromCollection, WithHeadings, ShouldAutoSize, WithEvents
{
    ...
    public function collection()
    {
        ...
        $news = News::join('details','details.news_id','=','news.id')
            ->select('news.title as title', 'news.created_at as created_at', 'details.instrument as instrument' , 'details.effect as effect',
                'details.comment as comment','news.text as text')
            ->orderBy('news.id', 'desc') // مشخص کردن جدول news برای id
            ->get();
